        ...
        <div className="relative flex w-full overflow-hidden">
          <div className="animate-marquee flex gap-24 items-center">
            {CLIENT_LOGOS.map((logo, idx) => (
              <div key={idx} className="flex items-center gap-2 px-4 py-2 text-zinc-700 font-sans font-semibold text-2xl hover:text-zinc-900 transition-colors">
                <span className="text-zinc-400 font-normal">{logo.symbol}</span>
                <span>{logo.name}</span>
              </div>
            ))}
            {CLIENT_LOGOS.map((logo, idx) => (
              <div key={`dup-${idx}`} className="flex items-center gap-2 px-4 py-2 text-zinc-700 font-sans font-semibold text-2xl hover:text-zinc-900 transition-colors">
                <span className="text-zinc-400 font-normal">{logo.symbol}</span>
                <span>{logo.name}</span>
              </div>
            ))}
          ...
          <div className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-[#fafafa] to-transparent pointer-events-none"></div>
          <div className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-[#fafafa] to-transparent pointer-events-none"></div>
        </div>
      </div>
